...
class SubIterationTeam(Protocol[TResult]):
...
class SubIterationJudge(Protocol[TResult]):
    """Protocol for a judge that evaluates the sub-task result."""

    async def assess(self, task: str, result: TResult, history: list[TResult]) -> JudgeAssessment:
